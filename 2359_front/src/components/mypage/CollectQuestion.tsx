...
const nonSelectBtn = `
  inline-block px-2.5 py-1 text-blue font-medium text-xs leading-tight rounded-xl shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg
  mr-3
`;
    ...
    <div>
      ...
      <div>
        {tagData.map((item) => {
          return (
            <TagButtons
              className={isSelect ? selectBtn : nonSelectBtn}
              key={item}
              type="button"
              onClick={() => handleTagName(item)}
            >
              {item}
            </TagButtons>
          );
